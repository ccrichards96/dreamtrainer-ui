...
const Testimonial: React.FC<{blok: TestimonialBlok}> = ({ blok }) => {
  ...
  switch (blok.card_style || 'default') {
    case 'minimal':
      ...
      break;
    case 'elevated':
      baseClasses.push('p-8', 'rounded-2xl', 'shadow-lg', 'hover:shadow-xl', 'transform', 'hover:-translate-y-2');
      break;
    default:
      baseClasses.push('p-8', 'rounded-2xl', 'shadow-lg', 'hover:shadow-xl', 'border', 'border-gray-100');
      break;
  }
  ...
  // Text color
  if (blok.text_color) {
    if (blok.text_color.startsWith('#') || blok.text_color.startsWith('rgb') || blok.text_color.startsWith('hsl')) {
  ...
  if (blok.border_color) {
    if (blok.border_color.startsWith('#') || blok.border_color.startsWith('rgb') || blok.border_color.startsWith('hsl')) {
  ...
  return (
    <div {...storyblokEditable(blok)} className={classes} style={customStyles}>
      ...
      <div className={`flex items-center ${textAlign === 'center' ? 'justify-center' : textAlign === 'right' ? 'justify-end' : 'justify-start'}`}>
