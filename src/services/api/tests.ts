 ...
 */
export const getTestAttemptsByCourse = async (courseId: string): Promise<TestAttemptData[]> => {
  try {
    const response = await apiClient.get<APIResponse<TestAttemptData[]>>(`/test-attempts/course/${courseId}`);
    return response.data.data;
  } catch (error) {
    if (error instanceof Error) {
      throw new Error(`Failed to get test attempts for course: ${error.message}`);
    }
    throw new Error('An unexpected error occurred while fetching test attempts');
  }
};
